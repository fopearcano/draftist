...
const projects = [
  ...
  { title: 'The Last Signal', meta: 'Novel · 18,420 words' },
  { title: 'Fox & Ember', meta: 'Novel · 31,090 words' },
];
const storyItems = [
...
const manuscript = `<h1>The Arrival</h1><p>The train arrived at <mark class="entity place" data-entity="Blackwater Bay">Blackwater Bay</mark> three hours late, dragging a ribbon of mist behind it.</p><p><mark class="entity character" data-entity="Elena Vale">Elena Vale</mark> stepped down onto the empty platform with one suitcase, a leather folio, and the uncomfortable sense that someone had been waiting for her. The <mark class="entity sound" data-entity="Station bell">station bell</mark> rang once.</p><p>Across the tracks, beyond the rusted iron fence, the town climbed the hillside in tiers of slate roofs and narrow chimneys. At the summit stood the <mark class="entity place" data-entity="Glass House">Glass House</mark>—a dark shape against a darker sky, all sharp angles and shuttered windows. <mark class="entity event" data-entity="The winter storm">The winter storm</mark> was already gathering beyond it.</p><h2>A familiar stranger</h2><p>“Miss Vale?”</p><p>The man beneath the station clock knew her name before she offered it. Somewhere nearby, a radio played <mark class="entity music" data-entity="Mother’s waltz">Mother’s waltz</mark>.</p><p>“<mark class="entity character" data-entity="Jonah Mercer">Jonah Mercer</mark>,” he said. “I knew your mother.”</p><h2>The letter</h2><p>He held out <mark class="entity object" data-entity="The letter">the letter</mark>. Her name crossed the envelope in her mother’s unmistakable hand.</p><h1>The Glass House</h1><p>By dusk, every window reflected the sea.</p>`;

const root = $('#root');
root.innerHTML = `
<div class="app view-contracted" data-theme="light">
  <header class="topbar">
  ...
  <div class="workspace">
    <aside class="left-panel">
      <div class="side-tabs"><button class="active" data-side="outline">Outline</button><button data-side="story">Story bible</button><button data-side="help">Help</button></div>
      <section class="side-view" id="outline-view"><div class="section-label"><span>DOCUMENT OUTLINE</span><button id="refresh-outline" title="Refresh outline" aria-label="Refresh outline">${svgIcon('refresh')}</button></div><nav id="outline-list"></nav><div class="outline-add"><button data-add-heading="h1">＋ Chapter</button><button data-add-heading="h2">＋ Section</button></div><p class="outline-help">Edit titles here, or use Chapter (H1) and Section (H2) in the manuscript toolbar.</p></section>
      ...
      <section class="side-view help-view" id="help-view" hidden><h2>Draftist help</h2><p>Write one continuous manuscript. Chapters and sections are built from heading styles and mirrored in the outline.</p><h3>Shortcuts</h3><dl><dt>⌘/Ctrl + S</dt><dd>Save now</dd><dt>⌘/Ctrl + 1</dt><dd>Chapter heading</dd><dt>⌘/Ctrl + 2</dt><dd>Section heading</dd><dt>⌘/Ctrl + Shift + F</dt><dd>Focus view</dd><dt>⌘/Ctrl + + / −</dt><dd>Text size</dd><dt>Escape</dt><dd>Exit focus</dd></dl><h3>Structure</h3><p>Rename headings directly in the outline. The manuscript and outline always reference the same heading elements.</p></section>
...
function renderStory() { const query=$('#story-search').value.toLowerCase(); const visible=storyItems.map(item=>({...item,uses:$$('.entity',editor).filter(mark=>mark.dataset.entity===item.name).length})).filter(item=>item.uses&&(currentFilter==='all'||item.type===currentFilter)&&item.name.toLowerCase().includes(query)); $('#story-list').innerHTML=visible.map(item=>`<button class="story-item" data-name="${escapeAttribute(item.name)}"><span class="story-glyph ${item.type}">${item.glyph}</span><span><b>${item.name}</b><small>${item.type} · ${item.uses} ${item.uses===1?'mention':'mentions'}</small></span><i class="tag-dot ${item.type}"></i></button>`).join('')||'<p class="empty">No matching elements in this manuscript.</p>'; $$('.story-item').forEach(button=>button.addEventListener('click',()=>{const match=$$('.entity',editor).find(mark=>mark.dataset.entity===button.dataset.name);if(match){scrollEditorTarget(match,true);match.classList.add('pulse');setTimeout(()=>match.classList.remove('pulse'),1200);}})); }
...
function setSide(name) { $$('[data-side]').forEach(b=>b.classList.toggle('active',b.dataset.side===name)); $('#outline-view').hidden=name!=='outline'; $('#story-view').hidden=name!=='story'; $('#help-view').hidden=name!=='help'; }
...
function setView(view){app.classList.remove('view-contracted','view-expanded','view-focus','view-focus-wide','view-typewriter','view-inverted','view-grayscale','view-retro','view-future');app.classList.add(`view-${view}`);$$('[data-view]').forEach(button=>button.classList.toggle('active',button.dataset.view===view));if(view==='typewriter')centerTypewriterCaret();}
function changeZoom(delta){const current=Number(editor.dataset.zoom||100);const zoom=Math.min(160,Math.max(70,current+delta));editor.dataset.zoom=zoom;editor.style.setProperty('--text-size',`${15*zoom/100}px`);$('#zoom-value').textContent=`${zoom}%`;}
...
$('#overall-italic').addEventListener('click',event=>{app.classList.toggle('overall-italic');event.currentTarget.classList.toggle('active');});
document.addEventListener('keydown',event=>{const command=event.metaKey||event.ctrlKey;if(event.key==='Escape'&&(app.classList.contains('view-focus')||app.classList.contains('view-focus-wide')))setView('contracted');if(!command)return;if(event.key.toLowerCase()==='s'){event.preventDefault();saveDraft();toast('Draft saved');}if(event.key==='1'){event.preventDefault();document.execCommand('formatBlock',false,'h1');updateOutline();}if(event.key==='2'){event.preventDefault();document.execCommand('formatBlock',false,'h2');updateOutline();}if(event.shiftKey&&event.key.toLowerCase()==='f'){event.preventDefault();setView('focus');}if(event.key==='+'||event.key==='='){event.preventDefault();changeZoom(10);}if(event.key==='-'){event.preventDefault();changeZoom(-10);}});
editor.addEventListener('input',()=>{updateStats();updateOutline();renderStory();saveDraft();centerTypewriterCaret();});
